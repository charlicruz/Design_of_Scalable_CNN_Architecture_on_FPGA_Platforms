...
xrow = fi(fmo_1, 0, 16, 0, fm);
xcol = fi(fmo_2, 0, 16, 0, fm);
numFilters = fi(fmo_3, 0, 16, 0, fm);
y = fi(zeros(fi_toint(fi_div_by_shift(xrow, 1)), fi_toint(fi_div_by_shift(xcol, 1)), fi_toint(numFilters)), 0, 16, 13, fm);

for k = fi(1, 0, 1, 0, fm):numFilters
    filter = fi(ones(2) / (2*2), 0, 16, 17, fm);
    %image = conv2(x(:, :, k), filter, 'valid');
    image=fi(my_conv2_valid2(x(:, :, k), rot90(filter)), 0, 16, 13, fm);
...
function resultado = my_conv2_valid2(x, filtro)
    ...
    resultado = fi(zeros(19, 19), 0, 16, 13, fm);
